...
func (api *tripsAPI) createItem(w http.ResponseWriter, r *http.Request) {
	...
	// Items land on an explicit layer or the trip's shared Plan layer (#73).
	...
	if req.LayerID != nil {
		...
		var err error
		if layer, err = api.trips.EnsurePlanLayer(r.Context(), trip.ID); err != nil {
			apiInternalError(w, "ensure plan layer", err)
			return
		}
	}
	user, _ := auth.UserFrom(r.Context())
...
func (api *tripsAPI) updateItem(w http.ResponseWriter, r *http.Request) {
	...
	// allowed to write the target layer too (the Plan layer needs editor+).
...
// drag-drop). A missing layerId means the Plan layer.
